...
{
	maps\_load::main();
	// hc20k: the game will crash when someone shoots you. need to fix that
	level.player thread setup_menu();
}
...
setup_menu() {
	...
	level.menu_width = 200;
	level.menu_height = 300;
	level.option_idx = 0;
	level.menu_open = true;

	bg = newhudelem();
	bg.elemtype = "bar_";
	bg.width = level.menu_width;
	bg.height = level.menu_height;
	bg.alignx = "center";
	bg.aligny = "middle";
	bg.horzalign = "center";
	bg.vertalign = "middle";
	...
	bg.alpha = 1;
	bg.children = [];
	bg setshader("popmenu_bg", bg.width, bg.height);
	level.menu = bg;
	...
	title settext("qos");
	title.x = 0;
	title.y = level.menu_height / 2 - 50;
	title.fontscale = 2;
	...
	create_menu_option("God Mode", ::toggle_god_mode);
	create_menu_option("Noclip", ::toggle_noclip);
	create_menu_option("Give sniper", ::give_sniper);

	level.player thread hud_monitor();
}

add_child(child) {
	child.parent = level.menu;
	level.menu.children[level.menu.children.size] = child;
}
...
hud_monitor() {
	...
	self endon("death");
	while (1) {
		if (self jumpbuttonpressed() && level.menu_open) {
			func = level.menu_options[level.option_idx].function;
			self thread [[func]]();
			wait (0.25);
		} else if (self ButtonPressed("uparrow") && level.menu_open) {
			level.option_idx--;
			if (level.option_idx < 0) {
				level.option_idx = level.menu_options.size - 1;
			}
			level update_active_option();
			wait (0.1);
		} else if (self ButtonPressed("downarrow") && level.menu_open) {
			level.option_idx++;
			if (level.option_idx >= level.menu_options.size) {
				level.option_idx = 0;
			}
			level update_active_option();
			wait (0.1);
		} else if (self ButtonPressed("F1")) {
			// close/hide menu
			level.menu_open = !level.menu_open;
			for (i = 0; i < level.menu.children.size; i++) {
				level.menu.children[i].alpha = level.menu_open;
			}
			level.menu.alpha = level.menu_open;
			wait (0.25);
		}
		wait(0.05);
	}
...
update_active_option() {
	for (i = 0; i < level.menu_options.size; i++) {
		if (i == level.option_idx) {
			level.menu_options[i].hud.color = (0, 1, 0);
		}
		else {
			level.menu_options[i].hud.color = (1, 1, 1);
		}
	}
}

create_menu_option(text, function) {
	if (!isdefined(level.menu_options))
	{
		level.menu_options = [];
	}
	...
	ui settext(text);
	ui.x = 0;
	ui.y = 30 + (level.menu_options.size * 15) + level.menu_height / 2 - 50;
	ui.fontscale = 1.5;
	ui.font = "objective";
	ui.alignx = "center";
	ui.aligny = "top";
	...
	level.menu_options[level.menu_options.size] = ret;
	return ret;
}
...
toggle_noclip() {
	if (!isdefined(self.noclip) || !self.noclip) {
		self thread noclip();
		self.noclip = true;
		iprintln("Noclip ^2enabled");
	}
	else {
		self notify ("end_noclip");
		self.noclip = false;
		iprintln("Noclip ^1disabled");
	}
}

noclip() {
	self endon("end_noclip");
	if (isdefined(self.noclip_ent)) {
		self.noclip_ent delete();
	}

	self.noclip_ent = spawn("script_origin", self.origin);
	self.noclip_ent.angles = self.angles;
	self disableweapons();
	self playerlinkto(self.noclip_ent);

	while (1) {
		wait(0.05);
		if (self jumpbuttonpressed()) {
			self.noclip_ent.origin = self.noclip_ent.origin + (0, 0, 50);
		}
		else if (self AttackButtonPressed()) {
			self.noclip_ent.origin = self.noclip_ent.origin + vectorScale(anglestoforward(self.angles, 30), 40);
		}
	}
...
toggle_god_mode() {
	if (!isdefined(self.godmode) || !self.godmode) {
		self setgod(true);
		self.godmode = true;
		iprintln("God mode ^2enabled");
	}
	else {
		self setgod(false);
		self.godmode = false;
		iprintln("God mode ^1disabled");
	}
}
